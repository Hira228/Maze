...
Model::Model()
    : maze(std::make_unique<typename s21::Maze>()),
      cave(std::make_unique<typename s21::Cave>()) {}
...
bool Model::ReadFromFileMaze(const std::string &path) {
  try {
    maze.reset(
        std::make_unique<typename s21::Maze>(FileManager<Maze>::Read(path))
            .release());
...
Model::get_data_maze() const {
  return maze->get_data();
}

pair Model::get_paramets() const noexcept { return maze->get_paramets(); }

cave_type Model::get_data_cave() const noexcept { return cave->GetCaveData(); }

pair Model::get_param_cave() const {
  return std::make_pair(cave->GetWeight() + 2, cave->GetHeight() + 2);
}

bool Model::GenerateCave(const value_type &rows, const value_type &cols,
...
bool Model::ReadFromFileCave(const std::string &path, const std::size_t &l_d,
                             const std::size_t &l_u, const std::size_t &b_d,
                             const std::size_t &b_u) {
  try {
    cave.reset(std::make_unique<typename s21::Cave>(
                   FileManager<Cave>::Read(path, l_d, l_u, b_d, b_u))
                   .release());
    return cave->GenerateCave();
  } catch (...) {
    return false;
