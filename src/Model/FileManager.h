...
template <typename Type> class FileManager;

template <> class FileManager<Maze> {
public:
  static Maze Read(const std::string path) {
    ...
    file >> rows >> cols;
    matrix temp_vertical(std::vector<std::vector<std::size_t>>(
        rows, std::vector<std::size_t>(cols, 0)));
    matrix temp_horizontal(std::vector<std::vector<std::size_t>>(
        rows, std::vector<std::size_t>(cols, 0)));
    ...
    file.close();
    return Maze(rows, cols, std::move(temp_vertical),
                std::move(temp_horizontal));
  }
...
template <> class FileManager<Cave> {
public:
  static Cave Read(const std::string path, const std::size_t &l_d,
    ...
    for (size_t i = 1; i < cave_.cave_.size() - 1; ++i) {
      for (size_t j = 1; j < cave_.cave_[i].size() - 1; ++j)
        file >> cave_.cave_[i][j];
    }
    file.close();
    return cave_;
  }
