...
View::View(Controller &&controller_, QWidget *parent)
    : QMainWindow(parent),
      controller(std::move(std::make_unique<s21::Controller>(
          std::forward<Controller>(controller_)))),
      ui(new Ui::View) {
  ui->setupUi(this);
  // maze
  ...
  connect(ui->btn_save_to_file, SIGNAL(clicked()), this,
          SLOT(SaveToFileMaze()));
  ...
  connect(ui->Button_Load_File_Cave, SIGNAL(clicked()), this,
          SLOT(LoadFromFileCave()));
  connect(ui->Button_Save_File_Cave, SIGNAL(clicked()), this,
          SLOT(SaveToFileCave()));
  connect(ui->btn_generate_cave_, SIGNAL(clicked()), this,
          SLOT(GenerateCave()));
  connect(ui->slow_automaric_button, SIGNAL(clicked()), this,
          SLOT(GenerateCaveSlow()));
  connect(ui->go_to_next_iteration_button, SIGNAL(clicked()), this,
          SLOT(DrawIteration()));
}
...
void View::LoadFromFile() {
  QString file_path = QFileDialog::getOpenFileName(
      this, "Choose file", "../../../../MazeExample",
      "All files(*.*);; txt(*.txt)");
  if (file_path.isEmpty())
    return;
  if (!controller->ReadFromFileMaze(file_path.toStdString()))
...
void View::SaveToFileMaze() {
  QString file_path =
      QFileDialog::getSaveFileName(this, "Save file", "../../../../MazeExample",
                                   "All Files(*);; txt(*.txt)");
  if (file_path.isEmpty())
    return;
  if (!controller->SaveToFileMaze(file_path.toStdString()))
    QMessageBox::critical(this, "Error", "Something went wrong...");
  else {
...
void View::LoadFromFileCave() {
  ...
  if (file_path.isEmpty())
    return;
  if (!controller->ReadFromFileCave(
          file_path.toStdString(), ui->spin_live_low->value(),
          ui->spin_live_up->value(), ui->spin_born_low->value(),
          ui->spin_born_up->value()))
    QMessageBox::critical(this, "Error", "Something went wrong...");
  else {
    QMessageBox::information(this, "Success", "Data downloaded successfully");
    ...
    if (ui->automatic_button->isChecked())
      DrawCave(controller->get_data_cave());
  }
}
void View::SaveToFileCave() {
  ...
  if (file_path.isEmpty())
    return;
  if (!controller->SaveToFileCave(file_path.toStdString()))
    QMessageBox::critical(this, "Error", "Something went wrong...");
  else {
    QMessageBox::information(this, "Success", "Data saved successfully");
  }
...
void View::GenerateMaze() {
  if (controller->GenerateMaze(
          static_cast<size_t>(ui->spin_box_rows->value()),
          static_cast<size_t>(ui->spin_box_cols->value()))) {
    DrawMaze();
  } else {
    QMessageBox::critical(this, "Error", "Something went wrong...");
  }
...
void View::SolvingMaze() {
  if (controller->SolvingMaze(
          std::make_pair(static_cast<size_t>(ui->spin_box_x_start->value()),
                         static_cast<size_t>(ui->spin_box_y_start->value())),
          std::make_pair(
              static_cast<size_t>(ui->spin_box_x_finish->value()),
              static_cast<size_t>(ui->spin_box_y_finish->value())))) {
    DrawSolvingMaze();
  } else {
    QMessageBox::critical(this, "Error", "Something went wrong...");
  }
...
void View::DrawMaze() {
  ...
  for (size_t i = 0; i != rows; ++i) {
    for (size_t j = 0; j != cols; ++j) {
      if (matrix_vetr[i][j])
        scene->addLine(QLineF(current_pos_x * (j + 1), current_pos_y * i,
                              current_pos_x * (j + 1),
                              current_pos_y * i + size_lines_vert),
                       pen);

      if (matrix_horz[i][j])
        scene->addLine(QLineF(current_pos_x * j, current_pos_y * (i + 1),
                              current_pos_x * j + size_lines_horz,
                              current_pos_y * (i + 1)),
                       pen);
    }
  }
...
void View::DrawSolvingMaze() {
  ...
  for (size_t i = 0; i != solving_path.size() - 1; ++i) {
    vec_line.push_back(scene->addLine(
        (solving_path[i].first + 0.5) * size_lines_horz,
        (solving_path[i].second + 0.5) * size_lines_vert,
        (solving_path[i + 1].first + 0.5) * size_lines_horz,
        (solving_path[i + 1].second + 0.5) * size_lines_vert, pen));
  }
}
    ...
    for (size_t j = 1; j != cols - 1; ++j) {
      if (matrix_cave[i][j]) {
        QGraphicsRectItem *rect = new QGraphicsRectItem(
            (j - 1) * cellWidth, (i - 1) * cellHeight, cellWidth, cellHeight);
        rect->setPen(pen);
...
void View::GenerateCave() {
  if (controller->GenerateCave(
          ui->spin_cave_rows_->value(), ui->spin_cave_cols_->value(),
          ui->spin_cave_life_chance->value(), ui->spin_live_low->value(),
          ui->spin_live_up->value(), ui->spin_born_low->value(),
          ui->spin_born_up->value())) {
    if (ui->automatic_button->isChecked())
      DrawCave(controller->get_data_cave());
...
void View::DrawIteration() {
  if (iter > controller->GetIterators().size() - 1)
    iter = 0;
...
void View::GenerateCaveSlow() {
  if (controller->GenerateCave(
    ...
    timer->start(ui->spin_cave_sleep->value()); // Устанавливаем интервал в
