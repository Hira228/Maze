...
namespace s21 {
Controller::Controller(std::unique_ptr<Model> model_)
...
pair Controller::get_paramets() const noexcept { return model->get_paramets(); }
std::pair<std::pair<matrix, matrix>, std::vector<std::pair<size_t, size_t>>>
Controller::get_data_maze() const {
...
bool Controller::ReadFromFileCave(const std::string &path,
                                  const std::size_t &l_d,
                                  const std::size_t &l_u,
                                  const std::size_t &b_d,
